...
from models import Plan, ProcessedWebhookEvent, Subscription, User
logger = logging.getLogger("answerspot.billing")
...
def handle_webhook(db: Session, payload: bytes, signature: str | None) -> None:
    ...
    event_id = event["id"]
    etype = event["type"]
    if db.query(ProcessedWebhookEvent).filter_by(event_id=event_id).first():
        logger.info("Stripe webhook %s (%s) already processed — skipping", event_id, etype)
        return
    db.add(ProcessedWebhookEvent(event_id=event_id, event_type=etype))
    db.commit()
    obj = event["data"]["object"]
    logger.info("Processing Stripe webhook %s (%s)", event_id, etype)
